...
from . import bitzlato_utils

_bob_logger = None


cdef class BitzlatoOrderBook(OrderBook):
    @classmethod
    ...
    def diff_message_from_exchange(cls,
        ...
        return OrderBookMessage(OrderBookMessageType.DIFF, {
            "trading_pair": bitzlato_utils.convert_from_exchange_trading_pair(msg["s"]),
            "update_id": msg["u"],
            "bids": msg["b"],
            "asks": msg["a"]
        }, timestamp=timestamp)
    ...
    def diff_message_from_db(cls, record: RowProxy, metadata: Optional[Dict] = None) -> OrderBookMessage:
        msg = ujson.loads(record["json"])  # Bitzlato json in DB is TEXT
        if metadata:
            msg.update(metadata)
        return OrderBookMessage(OrderBookMessageType.DIFF, {
            "trading_pair": bitzlato_utils.convert_from_exchange_trading_pair(msg["s"]),
