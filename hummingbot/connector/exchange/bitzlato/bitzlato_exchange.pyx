...
from bitzlato import client as bitzlato_client_module
from bitzlato.exceptions import BitzlatoAPIException
...
from hummingbot.connector.exchange.bitzlato.bitzlato_api_order_book_data_source import BitzlatoAPIOrderBookDataSource
...
from .bitzlato_order_book_tracker import BitzlatoOrderBookTracker
from .bitzlato_user_stream_tracker import BitzlatoUserStreamTracker
from .bitzlato_time import BitzlatoTime
from .bitzlato_in_flight_order import BitzlatoInFlightOrder
from .bitzlato_utils import (
...
cdef class BitzlatoExchangeTransactionTracker(TransactionTracker):
    cdef:
        BitzlatoExchange _owner

    def __init__(self, owner: BitzlatoExchange):
        super().__init__()
        self._owner = owner
...
cdef class BitzlatoExchange(ExchangeBase):
    ...
    BINANCE_TRADE_TOPIC_NAME = "bitzlato-trade.serialized"
    BINANCE_USER_STREAM_TOPIC_NAME = "bitzlato-user-stream.serialized"
    ...
    def __init__(self,
                 bitzlato_api_key: str,
                 bitzlato_api_secret: str,
                 trading_pairs: Optional[List[str]] = None,
                 trading_required: bool = True,
                 **dummy
                 ):

        self.monkey_patch_bitzlato_time()
        super().__init__()
        self._trading_required = trading_required
        self._order_book_tracker = BitzlatoOrderBookTracker(trading_pairs=trading_pairs)
        self._bitzlato_client = BitzlatoClient(bitzlato_api_key, bitzlato_api_secret)
        self._user_stream_tracker = BitzlatoUserStreamTracker(bitzlato_client=self._bitzlato_client)
        self._ev_loop = asyncio.get_event_loop()
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, BitzlatoInFlightOrder]
        self._order_not_found_records = {}  # Dict[client_order_id:str, count:int]
        self._tx_tracker = BitzlatoExchangeTransactionTracker(self)
    ...
    @property
    def bitzlato_client(self) -> BitzlatoClient:
        return self._bitzlato_client
    ...
    @property
    def in_flight_orders(self) -> Dict[str, BitzlatoInFlightOrder]:
    ...
    @property
    def order_book_tracker(self) -> BitzlatoOrderBookTracker:
        return self._order_book_tracker

    @property
    def user_stream_tracker(self) -> BitzlatoUserStreamTracker:
        return self._user_stream_tracker

    def restore_tracking_states(self, saved_states: Dict[str, any]):
        self._in_flight_orders.update({
            key: BitzlatoInFlightOrder.from_json(value)
            for key, value in saved_states.items()
        })

    async def get_active_exchange_markets(self) -> pd.DataFrame:
        return await BitzlatoAPIOrderBookDataSource.get_active_exchange_markets()

    def monkey_patch_bitzlato_time(self):
        if bitzlato_client_module.time != BitzlatoTime.get_instance():
            bitzlato_client_module.time = BitzlatoTime.get_instance()
            BitzlatoTime.get_instance().start()

    async def schedule_async_call(
            self,
            coro: Coroutine,
            timeout_seconds: float,
            app_warning_msg: str = "Bitzlato API call failed. Check API key and network connection.") -> any:
    ...
    async def query_api(
            ...
            func,
            *args,
            app_warning_msg: str = "Bitzlato API call failed. Check API key and network connection.",
        ...
        async with self._throttler.weighted_task(request_weight=request_weight):
            try:
                ...
                if "Timestamp for this request" in str(ex):
                    self.logger().warning("Got Bitzlato timestamp error. "
                                          "Going to force update Bitzlato server time offset...")
                    bitzlato_time = BitzlatoTime.get_instance()
                    bitzlato_time.clear_time_offset_ms_samples()
                    await bitzlato_time.schedule_update_server_time_offset()
                raise ex
    ...
    async def _update_balances(self):
        ...
        account_info = await self.query_api(self._bitzlato_client.get_account)
    ...
    async def _update_trade_fees(self):
        ...
        if current_timestamp - self._last_update_trade_fees_timestamp > 60.0 * 60.0 or len(self._trade_fees) < 1:
            try:
                res = await self.query_api(self._bitzlato_client.get_trade_fee)
                ...
                self.logger().network("Error fetching Bitzlato trade fees.", exc_info=True,
                                      app_warning_msg=f"Could not fetch Bitzlato trading fees. "
    ...
    cdef object c_get_fee(self,
        ...
        if order_type.is_limit_type() and fee_overrides_config_map["bitzlato_maker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["bitzlato_maker_fee"].value / Decimal("100"))
        if order_type is OrderType.MARKET and fee_overrides_config_map["bitzlato_taker_fee"].value is not None:
            return TradeFee(percent=fee_overrides_config_map["bitzlato_taker_fee"].value / Decimal("100"))

        if trading_pair not in self._trade_fees:
            # https://www.bitzlato.com/en/fee/schedule
        ...
        return estimate_fee("bitzlato", is_maker)
    ...
    async def _update_trading_rules(self):
        ...
        if current_tick > last_tick or len(self._trading_rules) < 1:
            exchange_info = await self.query_api(self._bitzlato_client.get_exchange_info)
    ...
    async def _update_order_fills_from_trades(self):
        cdef:
            ...
            # in case Bitzlato's user stream events are not working.
            # This is separated from _update_order_status which only updates the order status without producing filled
            # events, since Bitzlato's get order endpoint does not return trade IDs.
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            tasks = [self.query_api(self._bitzlato_client.get_my_trades, symbol=convert_to_exchange_trading_pair(trading_pair))
    ...
    async def _update_order_status(self):
        cdef:
            # This is intended to be a backup measure to close straggler orders, in case Bitzlato's user stream events
            # are not working.
            # The minimum poll interval for order status is 10 seconds.
            int64_t last_tick = <int64_t>(self._last_poll_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL)
            int64_t current_tick = <int64_t>(self._current_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL)
        ...
        if current_tick > last_tick and len(self._in_flight_orders) > 0:
            ...
            tasks = [self.query_api(self._bitzlato_client.get_order,
            ...
            for order_update, tracked_order in zip(results, tracked_orders):
                ...
                order_type = BitzlatoExchange.to_hb_order_type(order_update["type"])
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Bitzlato. Check API key and network connection."
                )
    ...
    async def _user_stream_event_listener(self):
        async for event_message in self._iter_user_event_queue():
            try:
                ...
                # Refer to https://github.com/bitzlato-exchange/bitzlato-official-api-docs/blob/master/user-data-stream.md
                # As per the order update section in Bitzlato the ID of the order being cancelled is under the "C" key
                if event_type == "executionReport":
                    ...
                    if execution_type == "TRADE":
                        order_filled_event = OrderFilledEvent.order_filled_event_from_bitzlato_execution_report(event_message)
                        ...
                        order_filled_event = order_filled_event._replace(trade_fee=self.c_get_fee(
                            tracked_order.base_asset,
                            tracked_order.quote_asset,
                            BitzlatoExchange.to_hb_order_type(event_message["o"]),
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching account updates.", exc_info=True,
                                      app_warning_msg="Could not fetch account updates from Bitzlato. "
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Bitzlato. "
    ...
    async def server_time(self) -> int:
        ...
        """
        result = await self.query_api(self._bitzlato_client.get_server_time)
    ...
    async def check_network(self) -> NetworkStatus:
        try:
            await self.query_api(self._bitzlato_client.ping)
        except asyncio.CancelledError:
            raise
        except Exception:
    ...
    @staticmethod
    def bitzlato_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(bitzlato_type: str) -> OrderType:
        return OrderType[bitzlato_type]
    ...
    async def create_order(self,
        ...
        price_str = f"{price:f}"
        type_str = BitzlatoExchange.bitzlato_order_type(order_type)
        side_str = BitzlatoClient.SIDE_BUY if trade_type is TradeType.BUY else BitzlatoClient.SIDE_SELL
        ...
        if order_type == OrderType.LIMIT:
            api_params["timeInForce"] = BitzlatoClient.TIME_IN_FORCE_GTC
        ...
        try:
            order_result = await self.query_api(self._bitzlato_client.create_order, **api_params)
            ...
            self.logger().network(
                f"Error submitting {side_str} {type_str} order to Bitzlato for "
                f"{amount} {trading_pair} "
                f"{price}.",
                exc_info=True,
    ...
    async def execute_cancel(self, trading_pair: str, order_id: str):
        try:
            cancel_result = await self.query_api(self._bitzlato_client.cancel_order,
                                                 symbol=convert_to_exchange_trading_pair(trading_pair),
                                                 origClientOrderId=order_id)
        except BitzlatoAPIException as e:
            if "Unknown order sent" in e.message or e.code == 2011:
                # The order was never there to begin with. So cancelling it is a no-op but semantically successful.
                self.logger().debug(f"The order {order_id} does not exist on Bitzlato. No cancellation needed.")
                self.c_stop_tracking_order(order_id)
                self.c_trigger_event(self.MARKET_ORDER_CANCELLED_EVENT_TAG,
        ...
        try:
            async with timeout(timeout_seconds):
                ...
                for cr in cancellation_results:
                    if isinstance(cr, BitzlatoAPIException):
                        continue
            ...
            self.logger().network(
                ...
                exc_info=True,
                app_warning_msg="Failed to cancel order with Bitzlato. Check API key and network connection."
            )
    ...
    cdef c_start_tracking_order(self,
                                ...
                                object amount,
                                object order_type):
        self._in_flight_orders[order_id] = BitzlatoInFlightOrder(
